<template>
    ...
    >
      <template v-if="!mobile" v-slot:opposite>
        ...
        >{{employer.employer}}</h2>
        <p class="text-overline">
            <v-btn v-if="i % 2 == 0" depressed plain :href="employer.companyProfile"><v-icon>mdi-linkedin</v-icon></v-btn> 
            {{employer.location}}
            <v-btn v-if="i % 2 != 0" depressed plain :href="employer.companyProfile"><v-icon>mdi-linkedin</v-icon></v-btn> 
        </p>
        <v-btn depressed plain @click="employer.expand = !employer.expand">See {{employer.expand ? 'Less &uarr;' : 'More &darr;'}}</v-btn>
        <v-expand-transition>
      ...
      <div v-for="(employment, j) in employer.employments" :key="j" class="py-4">
        <h2 class="headline font-weight-light mb-4">
          {{employment.jobTitle}} 
            <v-tooltip right v-if="employment.attachment">
                <template v-slot:activator="{on, attrs}">
                    <v-btn v-on="on" v-bind="attrs" :href="employment.attachment.link" target="_blank" depressed plain>
        ...
        <p class="text-subtitle-2">{{formatDate(employment.startDate)}} - {{formatDate(employment.endDate)}}</p>
        <ul>
...
<script>
    export default {
                        ...
                        employments: [
                            {
                                ...
                                attachment: {link: "DarianReck_LetterOfRecommendations_Avacon_GER.pdf", title: "Letter Of Recommendations (German)"}                    
                            }],
                    ...
                    },
                    {
                        ...
                        employments: [
                            ...
                            },
                            {
                                ...
                                attachment: {link: "DarianReck_LetterOfRecommendations_CODE_GER.pdf", title: "Letter Of Recommendations (German)"}
                            }],
                        location: "Berlin, Germany",
        ...
        created() {
            ...
            if(process.client) this.windowMobilePoller = setInterval(() => {
